...
def save_imgs(img, msk, msk_pred, i, save_path, datasets, threshold=0.5, test_data_name=None, file_name=None):
    # Convert tensor to numpy array and scale the image
    img = img[0].squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
    #img = img / 255. if img.max() > 1.1 else img
    
    # Handling for different dataset types
    if datasets == 'retinal':
    ...
    # Define file names
    base_name = file_name if file_name else f'image_{i}'
    img_file = os.path.join(save_path, f'{base_name}_img.png')
    msk_file = os.path.join(save_path, f'{base_name}_msk.png')
    msk_pred_file = os.path.join(save_path, f'{base_name}_msk_pred.png')
    #print(img.shape)
    #first_channel = img[:, :, 0]
    # Save each image separately
    #plt.imsave(img_file, first_channel)
    # plt.imsave(img_file, img)
    # #plt.imsave(img_file, img.squeeze(), cmap='gray')         #Need to add cmap='gray' in case of one channel images 
    # plt.imsave(msk_file, msk, cmap='gray')
    # plt.imsave(msk_pred_file, msk_pred, cmap='gray')
    plt.imsave(img_file, np.clip(img.squeeze(), 0, 1), cmap='gray')
    plt.imsave(msk_file, np.clip(msk, 0, 1), cmap='gray')
    plt.imsave(msk_pred_file, np.clip(msk_pred, 0, 1), cmap='gray')
    
    #print(f'Saved images: {img_file}, {msk_file}, {msk_pred_file}')
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2, alpha=0.25):
        ...
        self.gamma = gamma
        self.alpha = alpha
        self.bce_loss = nn.BCELoss(reduction='none')  #  BCE without reduction for focal computation

    def forward(self, pred, target):
        size = pred.size(0)
        pred_ = pred.view(size, -1)
        target_ = target.view(size, -1)
        bce = self.bce_loss(pred_, target_)
        pt = torch.where(target_ == 1, pred_, 1 - pred_)  # Probability for the true class
...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], 
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            net.eval()
            with torch.no_grad():
                outputs, outputs2, outputs3  = net(input)
    ...
    #         2: [0, 255, 0],     # class 2
    #         3: [0, 255, 255],     # class 2
    #         # ... up to number of classes
    #     }
    #     colored_pred = np.zeros((*prediction.shape, 3), dtype=np.uint8)
    #     for class_id, color in color_map.items():
    #         colored_pred[prediction == class_id] = color
    #     cv2.imwrite(test_save_path + '/' + case + '.png', colored_pred)
