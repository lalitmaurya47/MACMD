...
def train_one_epoch_isic(train_loader,
    ...
    for iter, data in enumerate(train_loader):
        ...
        if config.amp:
            ...
            # print("Model output shape:", out2.shape)
            # print("Model output shape:", out3.shape)
            # print("Target min/max:", targets.min(), targets.max())
            loss1 = criterion(out, targets)
            loss2 = criterion(out2, targets)
            loss3 = criterion(out3, targets)
            loss = loss1 + loss2 + loss3
...
def train_one_epoch_sy_ac(train_dataset,
                    train_loader,
                    model,
                    criterion, 
                    optimizer, 
    ...
    for iter, data in enumerate(train_loader):
        ...
        #     metric_i = test_single_volume(images, targets, model, classes=config.model_config['num_classes'], patch_size=[config.input_size_h, config.input_size_w],
        #                             test_save_path=None, case=None, z_spacing=config.z_spacing, val_or_test=False)
        #     metric_list += np.array(metric_i)
        #     i_batch += 1

        # metric_list = metric_list / len(train_dataset)
    ...
    scheduler.step()
    etime = time.time()
    log_info = f'Finish one epoch train: epoch {epoch}, loss: {mean_loss:.4f},train_dice: {mean_loss.item():.6f}, time(s): {etime-stime:.2f}'
    print(log_info)
    logger.info(log_info)
